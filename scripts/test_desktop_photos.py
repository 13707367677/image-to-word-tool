...
def add_cell_image(cell, img_path, width_in):
    """Via temp PNG file to bypass EXIF bugs"""
    import tempfile
    p = cell.paragraphs[0]
    ...
    with PILImage.open(img_path) as pil_img:
        ...
        try:
            ...
            print(f'    Temp PNG: {os.path.getsize(tmp_path)} bytes')
            run.add_picture(tmp_path, width=Inches(width_in))
            print(f'    add_picture OK')
        finally:
...
while pair_idx * 2 < n:
    ...
    while pair_idx * 2 < group_end:
        ...
        print(f'\nAdding pair {pair_idx}: left={left_i} right={right_i} has_right={has_right}')
        ...
        print(f'  Left: {os.path.basename(photos[left_i])}')
        add_cell_image(img_row.rows[0].cells[0], photos[left_i], max_w_in)
        if has_right:
            print(f'  Right: {os.path.basename(photos[right_i])}')
...
out = os.path.join(os.environ['TEMP'], 'desktop_photos_test.docx')
doc.save(out)
print(f'\nSaved: {out}')
